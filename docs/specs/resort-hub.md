...
| 機能 | 内容 | 時期 |
|---|---|---|
| **距離表示** | 「現在地から 142km」をヘッダーに出す。端末の位置情報のみで計算でき通信不要。`locations` 全件の最小距離を採る | v1（安い） |
| **マップアプリ起動** | ⋯メニューから座標で Apple/Google マップを開く | v1 |
| **近くのゲレンデ** | 全件を端末側で距離計算。サーバー側の範囲クエリは §7.3(1) の理由で不可 | v1.5 |
| **チェックインの真正性** | 端末位置と `locations` の最小距離で「実際にそこにいる」ことを検証 | v1.5 |
...
```
{ゲレンデマスタ}/{resortId}                  … ★既存。§7.2 の実スキーマ。アプリからは読み取り専用
...
マスタ:  isActive ASC, area ASC                            （コンディション画面の一覧）
```
...
- **ゲレンデマスタ**: 読み取りのみ許可。**書き込みはクライアントからも Functions からも禁止**
...
1. **ゲレンデマスタのコレクション名** — スキーマは共有済み。名前だけ未確認
2. **`locations` の実態** — 2件以上持つドキュメントは実在するか。全ドキュメントに
   最低1件の座標が入っているか（0件があるなら §7.3(2) のガードが必須）
3. **`isActive` と `isPermanentlyClosed` の使い分け** — 前者が「今季休業」、後者が「廃業」
   という理解で合っているか（§7.3(5) の表はこの前提）
4. **ドキュメントIDの採番規則** — `101001` は地域コード＋連番か。規則があるなら
   区切りをドキュメント化しておきたい（アプリ側は不透明IDとして扱うので実装には影響しない）
5. **用語統一** — `posts.mountainId` → `resortId` のリネームを実施してよいか
6. **利用する外部APIの具体サービス** — 料金・レート制限・出典表記義務・積雪のカバー範囲。
   リフト稼働は取得できない前提で、ユーザー投稿タグで補う方針の可否
7. **想定書き込み量** — 1ゲレンデ1日あたり数件か数百件か。数百件ならチャット型UIを再検討
8. **未ログインでの閲覧可否**
