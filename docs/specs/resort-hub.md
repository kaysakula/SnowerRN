...
作成: 2026-08-28 / ステータス: **v1 設計確定。実装着手済み（§10-1 完了）**
...
| D8 | コレクション名 | **`resorts`** |
| D9 | 代表座標 | **`locations[0]` が主要ポイント**。常にこれを代表点として使う |
...
#### (2) 代表点は常に `locations[0]`（D9）
`locations` は複数件を持つゲレンデがあるが、**0番目が主要ポイント**として登録されている。

- **距離表示・マップ起動・天気APIの問い合わせは、すべて `locations[0]` を使う**。
  「全件の最小距離」のような凝った計算はしない（主要ポイントからの距離が期待される答え）
- 1件目以外は v1 では使わない。情報タブ（v1.5）で「他のベース」として一覧表示する余地はある
...
| 機能 | 内容 | 時期 |
|---|---|---|
| **距離表示** | 「現在地から 142km」をヘッダーに出す。端末の位置情報のみで計算でき通信不要。`locations[0]` からの距離 | v1（安い） |
| **マップアプリ起動** | ⋯メニューから座標で Apple/Google マップを開く | v1 |
| **近くのゲレンデ** | 全件を端末側で距離計算。サーバー側の範囲クエリは §7.3(1) の理由で不可 | v1.5 |
| **チェックインの真正性** | 端末位置と `locations[0]` の距離で「実際にそこにいる」ことを検証。判定半径は広めに取る（大型ゲレンデは主要ポイントから数km離れた地点でも滑走中） | v1.5 |
...
1. **前提整備**
   - ~~`2dc181f` のディレクトリ移動で壊れたインポートパス14件の修正~~ ✅ 完了
   - `DocumentSnapshot.exists` の誤用修正（§10.1）— **未着手・要判断**
...
4. `ResortModel` の定義（§7.2）と `resortConditions` / `resortStats` コレクションの新設
...
### 10.1 着手前に直すべき既存バグ（インポート修正で表面化）

`@react-native-firebase` v23 では `DocumentSnapshot.exists` は**プロパティではなくメソッド**。
現在の `timelineService.ts` はプロパティとして参照しているため、常に truthy になる。

| 箇所 | 現状 | 影響 |
|---|---|---|
| `timelineService.ts:58, 97` | `if (userDoc.exists)` | 常に true。存在しないユーザーで `data()` が undefined になり得る |
| `timelineService.ts:196, 211` | `return likeDoc.exists` | **関数オブジェクトを返す**ため、いいね／ブックマーク判定が常に true |

修正は `.exists` → `.exists()` の4箇所のみ。リゾートハブでも同じ Firestore 参照を
書くことになるため、**新規コードを書く前に直しておく**（同じ誤用をコピーしないため）。

その他、型チェックで既存の指摘が数件出ている（`react-native-reanimated` 未導入、
`SignupNavigator` の default import、`locales/index.ts` の型など）。
リゾートハブとは独立しているため、別途対応。
...
1. **用語統一** — `posts.mountainId` → `resortId` のリネームを実施してよいか。
   既存の投稿データにも影響するため、投稿数が少ない今が最後のタイミング
2. **既存バグの修正着手可否** — §10.1 の `.exists` 4箇所
